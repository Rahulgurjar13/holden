    ...
    <footer className="bg-navy-deep text-paper/85 relative overflow-hidden">
      <div className="container relative pt-20 pb-8">
        ...
        <div className="grid gap-12 lg:gap-16 md:grid-cols-2 lg:grid-cols-4">
          <div>
            <h4 className="font-mono text-[10px] uppercase tracking-[0.25em] text-accent mb-5">Products</h4>
            <ul className="space-y-3 text-[15px] font-serif">
              {PRODUCTS.map((p) => (
                <li key={p.slug}>
                  <Link to={`/products/${p.slug}`} className="text-paper/75 hover:text-paper transition-colors">
                    {p.name}
                  </Link>
                </li>
              ))}
          ...
          <div>
            <h4 className="font-mono text-[10px] uppercase tracking-[0.25em] text-accent mb-5">Services</h4>
            <ul className="space-y-3 text-[15px] font-serif">
              {SERVICES.map((s) => (
                <li key={s.slug}>
          ...
          <div>
            ...
            <ul className="space-y-4 text-[15px]">
              <li>
                <a href={`tel:${CONTACT.phone}`} className="font-serif text-paper hover:text-accent transition-colors block">
                  {CONTACT.phone}
                </a>
                <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-paper/50">Direct line</span>
              </li>
              <li>
                <a href={`mailto:${CONTACT.email}`} className="font-serif text-paper hover:text-accent transition-colors block">
                  {CONTACT.email}
                </a>
                <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-paper/50">Enquiries</span>
              </li>
              <li>
                <span className="font-serif text-paper block leading-snug">{CONTACT.address}</span>
                <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-paper/50">Registered office</span>
              </li>
              <li>
                <Link to="/contact" className="link-underline font-mono text-[11px] uppercase tracking-[0.2em] text-paper hover:!text-accent">
        ...
        <div className="mt-20 pt-6 border-t border-paper/10 flex flex-col md:flex-row items-center justify-between gap-3 font-mono text-[10px] uppercase tracking-[0.2em] text-paper/50">
          <p>© {new Date().getFullYear()} Ghodu Infotech Pvt Ltd · All rights reserved</p>
          <p>Designed with care · Holden India</p>
        </div>
      </div>
    </footer>
  );
